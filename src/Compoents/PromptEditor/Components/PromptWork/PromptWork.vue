  ...
  <div class="PromptWork" :class="{ isPNGExporting }" @click="onWorkClick">
    <div class="AddArea Area">
      ...
      <div class="options">
        ...
        <div class="line">
          <button @click="copy(outputText)" title="复制"><Icon icon="radix-icons:clipboard-copy" /> 复制</button>

          <div class="button-group">
            <button @click="doDisableAll()" v-tooltip="'全部禁用'" class="icon">
              <Icon icon="radix-icons:shadow-none" />
            </button>
            <button @click="doSwitchIO()" v-tooltip="'用输出替换输入'" class="icon" :class="{ disabled: outputText == inputText }">
              <Icon icon="radix-icons:arrow-up" />
            </button>
            <button @click="doClear()" v-tooltip="'清空输入'" class="icon" :class="{ disabled: inputText?.length == '' }">
    ...
    <div class="main" ref="main">
      <div class="PromptListArea Area" :class="[promptWork.groups.length <= 1 ? 'noGroup' : 'hasGroup']" @contextmenu.prevent>
        <div class="PromptGroup" v-for="group in promptWork.groups">
          ...
          <PromptList v-for="promptList in group.lists" :key="promptList.data.id" :list="promptList" @update="doExportPrompt()">
            <div class="name-bar">
