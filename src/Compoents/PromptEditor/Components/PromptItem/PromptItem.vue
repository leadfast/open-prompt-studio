...
<template>
  <div
    ...
    :class="[`type-${item.data.word.type}`, `subType-${item.data.word.subType}`, { disabled: item.data.disabled, isEdit: item.state.isEdit }]"
    @click="onClick"
  ...
  >
    <div class="content">
      <div class="displayName name">
        <span v-if="item.state.isEdit == 'text'" ref="input" class="input" contenteditable="true" @keydown="onKeydown" @blur="onBlur">{{ displayName }}</span>
        <span v-else>{{ displayName }}</span>
        <div class="lv" v-if="displayLv">{{ displayLv }}</div>
      </div>
      <div class="langName name" v-if="langName || item.state.isEdit == 'lang'">
        <span v-if="item.state.isEdit == 'lang'" ref="inputLang" class="input inputLang" contenteditable="true" @keydown="onKeydownLang" @blur="onBlurLang">{{ langName }}</span>
