...
<template>
  <div class="PromptDict">
    <div class="notion-settings" :class="{ isHoverButton }">
      <button class="notion-me" @click="doGotoNotionMe" @mousemove="setNotionHover(true)" @mouseleave="setNotionHover(false)">
      ...
      <div class="notion-config">
        ...
        <div class="line buttons">
          <button class="full" :class="{ disabled: !notioConfigActive || loading || !enableMyNotion }" @click="reloadData()">
            {{ loading ? "载入中..." : "载入" }}
          </button>
        </div>
      </div>
    ...
    async fetchNotion() {
      try {
        ...
        if (/Make sure the relevant pages and databases are shared with your integration/.test(e.message)) {
          ...
          alert(`Notion 连接错误：没有 Notion 数据库的访问权限。请在此 Notion 数据库页面菜单的 'Connections' 中添加你的集成应用`);
        }
      } finally {
        this.loading = false;
      }
